...
public class SummaryController implements Initializable{
    ...
    public void initialize(URL url, ResourceBundle rb) {
        Platform.runLater(() -> {
            ...
            for(Transaction item : collectiveData.getTransaction()) {
                int monthValue = LocalDate.parse(item.getTransaction_date()).getMonthValue();
                if (currMonthVal == monthValue) {
                    for(TransactionDetail transactionDetail : collectiveData.getTransactionDetail()) {
                        if (item.getTransaction_id() == transactionDetail.getTransaction_id()) {
                            countOfUnits+=transactionDetail.getTransaction_quantity();
                        }
                    }
                }
            }
